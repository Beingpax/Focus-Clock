...
function nextTrack(){if(!playlist.length)return;if(shuffle){let next=currentIndex;while(playlist.length>1&&next===currentIndex)next=Math.floor(Math.random()*playlist.length);loadIndex(next)}else loadIndex(currentIndex+1)}
...
ui.shuffle.addEventListener('click',()=>{shuffle=!shuffle;ui.shuffle.classList.toggle('is-on',shuffle);ui.shuffle.setAttribute('aria-pressed',String(shuffle))});
...
selectMode('clock',true);setInterval(updateClock,200);loadYouTubeAPI();if(location.protocol==='file:')ui.notice.hidden=false;
